...
public class Equation
{
  ...
  public Equation(char[] variables)
    throws Exception
  {
  ...
  public Equation(String equation, char[] variables)
    throws Exception
  {
  ...
  private double evaluateArithmetic(String eq)
    throws Exception
  {
    ...
    for (int x = 0; x < eq.length(); x++)
    {
      if (eq.charAt(x) == '+')
      {
        ...
        String rtrnString = eq.substring(0, a[0]) + String.valueOf(evaluateStri(term1) + evaluateStri(term2)) + 
          eq.substring(b[1] + 1);
  ...
  private double evaluateExponent(String eq)
    throws Exception
  {
  ...
  private double evaluateMultDevi(String eq)
    throws Exception
  {
  ...
  private double evaluatePare(String eq)
    throws Exception
  {
  ...
  private double evaluateStri(String eq)
    throws Exception
  {
    ...
    this.log.warning(eq + 
      " was recognized as not being a number but an operation cannot be identified. returning Double.valueOf(eq)"
      .toUpperCase());
    return Double.valueOf(eq).doubleValue();
  }
