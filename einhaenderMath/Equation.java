...
// TODO here's a theory. I think I heard reference on the internet to using strings to identify
// methods (although I recal that it was laggy). So is it possibly to compile a string to a method?
// if so that would be SO much easier. and probably less laggy, depending on how laggy compiling is.
...
public class Equation {
  ...
  private double evaluateArithmetic(String eq) throws Exception {
    ...
    for (int x = 0; x < eq.length(); x++) {
      if (eq.charAt(x) == '+') {
        ...
        String rtrnString = eq.substring(0, a[0])
          + String.valueOf(evaluateStri(term1) + evaluateStri(term2)) + eq.substring(b[1] + 1);
  ...
  private double evaluateExponent(String eq) throws Exception {
    ...
    for (int x = 0; x < eq.length(); x++) {
      if (eq.charAt(x) == '^') {
        ...
        String term2 = eq.substring(b[0], b[1] + 1);
        String rtrnString = eq.substring(0, a[0])
          + String.valueOf(Math.pow(evaluateStri(term1), evaluateStri(term2)))
          + eq.substring(b[1] + 1);
        return evaluateStri(rtrnString);
      }
    }
  ...
  private double evaluateStri(String eq) throws Exception {
    ...
    log.warning(
      eq + " was recognized as not being a number but an operation cannot be identified. returning Double.valueOf(eq)"
        .toUpperCase());
    return Double.valueOf(eq).doubleValue();
  }
    ...
    if ((isInArray(chrI, OPERATORS)) && ((i != 0) || (chrI != '-'))) {
      if ((i == 0) || (chrI != '-') || (!isInArray(eq.charAt(i - 1), OPERATORS))) {
        throw new IllegalArgumentException(
          "given argument '" + i + "' corresponds to the character '" + eq.charAt(i)
  ...
  private void reformat() throws Exception {
    ...
    for (int x = 0; x < equationReformated.length(); x++) {
      if (equationReformated.charAt(x) == '-') {
        equationReformated = (equationReformated.substring(0, x) + "-1*"
          + equationReformated.substring(x + 1));
      }
    }
    for (int x = 0; x < equationReformated.length(); x++) {
    ...
    for (int x = 0; x < equationReformated.length(); x++) {
      ...
      if (parenthesisCount < 0) {
        throw new Exception("invalid equation " + equationOrigional
          + ": close parenthesis at index " + x + " has no corresponding open parenthisis");
      }
    }
    if (parenthesisCount != 0) {
      throw new Exception(
        "invalid equation " + equationOrigional + ": parenthisies were unbalenced");
    }
    for (int x = 0; x < equationReformated.length(); x++) {
